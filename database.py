...
def get_pending_reminders() -> list:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, user_id, text, remind_at, notified_10, notified_5, notified_2, is_sent FROM reminders WHERE is_sent = FALSE"
    )
    rows = cursor.fetchall()
    conn.close()
...
def mark_notified(reminder_id: int, level: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE reminders SET notified_" + level + " = TRUE WHERE id = %s",
        (reminder_id,)
    )
    conn.commit()
    conn.close()
...
def snooze_reminder(reminder_id: int, new_time: datetime):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE reminders SET remind_at = %s, notified_10 = FALSE, notified_5 = FALSE, notified_2 = FALSE, is_sent = FALSE WHERE id = %s",
