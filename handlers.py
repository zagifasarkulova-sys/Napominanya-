...
async def got_warn(callback: CallbackQuery, state: FSMContext):
    ...
    await save_reminder(callback.message, state, warn_before, edit=True)
    await callback.answer()
...
async def got_custom_warn(message: Message, state: FSMContext):
    try:
        ...
        if warn_before <= 0:
            ...
            return
        await save_reminder(message, state, warn_before, edit=False)
    except ValueError:
        await message.answer("❌ Только цифры! Например: <code>20</code>", parse_mode="HTML")


async def save_reminder(message_or_callback, state: FSMContext, warn_before: int, edit: bool):
    data = await state.get_data()
    remind_at = datetime.strptime(data["remind_at"], "%Y-%m-%d %H:%M")
    reminder_id = add_reminder(message_or_callback.chat.id if hasattr(message_or_callback, 'chat') else message_or_callback.from_user.id, data["text"], remind_at, warn_before)
    await state.clear()
